...
vec3 trace(Ray ray, Sphere sphere, Light light){ //Work on this
	...
	if(rayDepth = 0){
		...
		vec3 V = intersectionPoint;
		vec3 N = sphere.get_normal(intersectionPoint);
		vec3 Li = intersectionPoint - light_object[0].origin;
		vec3 R = 2*(N * Li)*N - Li;
...
void render(){
	...
	for(int x=-WINDOW/2; x < WINDOW/2; x++){
		float xN=x * LIMIT*LIMIT*700;
		// printf("xN: %f\n", xN); 
		for (int y = -WINDOW/2; y < WINDOW/2; y++)
		{
		float yN=y * LIMIT*LIMIT*700;
			// glBegin(GL_POINTS);
...
void callbackDisplay() {
	...
	vec3 lightColor(2.5, 2.5, 2.5);
	Light light(lightPos, lightColor);
	light_object[0] = light;

	glTranslatef(0.0, 0.0, -20.0);
	vec3 center_ball1(-1.0, 2.0, 0.0);
	float radius_ball1 = 5.0;
